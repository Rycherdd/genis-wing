-- Corrigir função para calcular horas de aprendizado corretamente
...
BEGIN
  ...
  -- Calcular horas de aprendizado com conversão correta
  SELECT COALESCE(
    SUM(
      EXTRACT(EPOCH FROM (aa.horario_fim::time - aa.horario_inicio::time)) / 3600
    ),
    0
  ) INTO v_horas_interval
